...
bot.on('message', async msg => {
  ...
  if (chatIdBlocked[chatId]) {
    bot.sendMessage(chatId, 'You are currently blocked from using the bot. Please contact @Nomadly_support', rem);
    return;
  }
...
Nomadly Bot`,
    ...
    if (message === 'Back') {
      bot.sendMessage(chatId, `Price of ${domain} subscription is ${priceOf[domain]} USD. Choose payment method.`, pay);
  ...
  //
    ...
    if (subscribedPlan) {
      if (!isSubscribed(chatId)) {
        bot.sendMessage(
          chatId,
          `Your ${subscribedPlan} subscription is expired on ${new Date(planEndingTime[chatId])}`,
        );
        return;
      }
